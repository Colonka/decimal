...
decimal number implemented as a four-element array of 32-bit unsigned integers (`unsigned int bits[4];`).
...
For computing arithmetic operations with decimal which result can overflow decimal that may lead to round-off error I use BigDecimal number.

big_decimal number implemented as a seven-element array of 32-bit unsigned integers (`unsigned int bits[7];`).

`bits[0]`, `bits[1]`, `bits[2]`, `bits[3]`, `bits[4]`, `bits[5]` contain the low, middle, and high 32 bits of the 96-bit integer number accordingly.

`bits[6]` contains the scale factor and sign, and consists of following parts:
- Bits 0 to 15, the lower word, are unused and must be zero.
- Bits 16 to 23 must contain an exponent between 0 and 28, which indicates the power of 10 to divide the integer number.
- Bits 24 to 30 are unused and must be zero.
- Bit 31 contains the sign; 0 meaning positive, and 1 meaning negative.
